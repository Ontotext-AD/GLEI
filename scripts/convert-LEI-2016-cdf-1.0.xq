...
declare function local:downloadURL($uri as xs:string, $record, $download as xs:string) {
    let $url := $download
    construct {
        <{$url}> a leio:DownloadFile;
                 leio:contentDate    {$record/gleif:ContentDate/text()}^^xsd:dateTime;
                 leio:downloadDate   {$record/gleif:LastSuccessfulDownloadDate/text()}^^xsd:dateTime;
                 leio:originator     {$record/gleif:LOULEI/text()};
                 leio:recordCount    {$record/gleif:RecordCount/text()}^^xsd:integer;
    }
};

declare function local:managingLOU($uri as xs:string, $record, $download as xs:string) {
    let $url := fn:concat($uri, $record/gleif:LOULEI/text())
    construct {
        <{$url}> a fibo-fbc-fct-breg:BusinessRegistrationAuthority;
                 leio:recordCount         {$record/gleif:RecordCount/text()}^^xsd:integer;
                 leio:downloadFile        <{$download}>;
                 fibo-fnd-rel-rel:manages <http://data.ontotext.com/resource/glei/>
    }
};
...
for $lou_file in doc($input)/lei:LEIData/lei:LEIHeader/lei:Extension/gleif:LOUSources/gleif:LOUSource
let $uri := "http://data.ontotext.com/resource/glei/"
let $download := fn:concat($uri, $lou_file/gleif:LOULEI, "-", fn:encode-for-uri($lou_file/gleif:ContentDate))
return (
    local:managingLOU($uri, $lou_file, $download),
    local:downloadURL("http://data.ontotext.com/resource/glei/", $lou_file, $download)
),
local:glei_master()
)
